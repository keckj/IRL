	...
	\begin{abstract}
		A miniature 3D tracked ultrasonic probe has been developped to aquire intra-articular cartilage images under artroscopic surgical conditions. The aim is to detect cartilaginous lesions (arthrosis) and quantify their precise sizes and locations to help the clinician in his diagnostic and his therapeutic decision making.
...
As PNN was the simplest algorithm, this is where we began. 
...
\For{each GPU i}{
	Allocate voxel subgrid;\\
	Allocate mean subgrid;\\
	Allocate hit subgrid;\\
	Allocate image array;\\
	Allocate 12 transformations arrays;\\
	Send images to device $i$ on stream $s_1^i$;\\
	Send transformations to device $i$ on stream $s_1^i$;\\
}
...
\For{each subgrid}{
	Compute bin-filling kernel on device $i$ on stream $s_i^{j\mod{2}}$;\\
